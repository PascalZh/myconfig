...
local function deepCopy(input)
  if type(input) == 'table' then
    local output = {}
    for i, o in next, input, nil do output[deepCopy(i)] = deepCopy(o) end
    return output
  else
    return input
  end
...
local function charRepetition(n, character)
  ...
  for _ = 1, n do s = s .. character end
  return s
end
...
local function functionReport(information)
  ...
  src = string.gsub(src, [[C:\Users\Pascal\AppData\Local\nvim.*data\site\pack\packer\start]], 'P')
...
function module.start()
  if not outputHeader then rebuildColumnPatterns() end
  reportCache = {}
  allReports = {}
  reportCount = 0
  startTime = getTime()
...
function module.report(filename)
  if not stopTime then module.stop() end
  ...
  if printFun ~= nil then printFun(totalTimeOutput) end
  ...
  for i = 1, reportCount do
    ...
    if funcReport.count > 0 and funcReport.timer <= totalTime then
      ...
      if config.outputFile ~= '' then
        if singleSearchReturn(funcReport.title, config.outputFile) then printThis = false end
      end
      if printThis then -- Remove lines that are not needed
        if singleSearchReturn(funcReport.title, '[[C]]') then printThis = false end
      end
      if printThis then
        ...
        if printFun ~= nil and verbosePrint then printFun(output) end
      end
    end
  end
  fileWriter:write(divider)
  fileWriter:close()
  if printFun ~= nil then printFun(config.reportSaved .. '\'' .. filename .. '\'') end
end
